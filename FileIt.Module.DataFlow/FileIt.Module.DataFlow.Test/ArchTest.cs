...
public class ArchitectureTests
{
    ...
    private static readonly Architecture Architecture = new ArchLoader()
        .LoadAssemblies(
            ...
            typeof(FileIt.Module.DataFlow.App.DataFlowEvents).Assembly,
            typeof(FileIt.Module.DataFlow.Host.DataFlowDeadLetterReader).Assembly
        )
        .Build();
    ...
    {
        var testLayer = Types().That().HaveFullNameContaining("FileIt.Domain.");
        var targetLayer = Types().That().HaveFullNameContaining("FileIt.Module.DataFlow.Host.");
    ...
    {
        var testLayer = Types().That().HaveFullNameContaining("FileIt.Domain.");
        var targetLayer = Types().That().HaveFullNameContaining("FileIt.Module.DataFlow.App.");
    ...
    public void Host_Should_Not_Have_Dependency_On_Domain()
    {
        ...
        var testLayer = Types().That().HaveFullNameContaining("FileIt.Module.DataFlow.Host.");
    ...
    {
        var testLayer = Types().That().HaveFullNameContaining("FileIt.Module.DataFlow.App.");
        var targetLayer = Types().That().HaveFullNameContaining("FileIt.Infrastructure.");
