...
public class DataFlowEvents
{
    ...
    public static EventId DataFlowWatcherAddRequestLog = new EventId(
        3001,
        nameof(DataFlowWatcherAddRequestLog)
    );

    // Fired when we move the file from source to working container
    public static EventId DataFlowWatcherMoveToWorking = new EventId(
        3002,
        nameof(DataFlowWatcherMoveToWorking)
    );

    // Fired when we put a message on the service bus queue to kick off the transform
    public static EventId DataFlowWatcherQueueTransform = new EventId(
        3003,
        nameof(DataFlowWatcherQueueTransform)
    );
    ...
    public static EventId DataFlowSubscriberReceive = new EventId(
        3011,
        nameof(DataFlowSubscriberReceive)
    );

    // Fired when receiving the message fails for any reason
    public static EventId DataFlowSubscriberReceiveFailed = new EventId(
        3012,
        nameof(DataFlowSubscriberReceiveFailed)
    );

    // Fired when we look up the original request log by correlation ID
    public static EventId DataFlowSubscriberGetRequestLog = new EventId(
        3013,
        nameof(DataFlowSubscriberGetRequestLog)
    );

    // Fired when we can't find the request log — something went wrong upstream
    public static EventId DataFlowSubscriberRequestLogNotFound = new EventId(
        3014,
        nameof(DataFlowSubscriberRequestLogNotFound)
    );

    // Fired when the blob name is missing from the request log record
    public static EventId DataFlowSubscriberBlobNameMissing = new EventId(
        3015,
        nameof(DataFlowSubscriberBlobNameMissing)
    );

    // Fired when we move the exported file from working to final container
    public static EventId DataFlowSubscriberMoveToFinal = new EventId(
        3016,
        nameof(DataFlowSubscriberMoveToFinal)
    );

    // Fired when we update the request log with the transform results
    public static EventId DataFlowSubscriberUpdateRequestLog = new EventId(
        3017,
        nameof(DataFlowSubscriberUpdateRequestLog)
    );

    // Fired when the whole flow completes successfully end to end
    public static EventId DataFlowSubscriberCompleted = new EventId(
        3018,
        nameof(DataFlowSubscriberCompleted)
    );
    ...
    public static EventId DataFlowTransformCompleted = new EventId(
        3021,
        nameof(DataFlowTransformCompleted)
    );
}
