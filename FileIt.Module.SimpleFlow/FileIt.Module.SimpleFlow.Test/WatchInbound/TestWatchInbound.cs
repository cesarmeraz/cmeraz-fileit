...
{
    public required Moq.Mock<ILogger<App.WatchInbound>> _loggerMock;
    public required Moq.Mock<IHandleFiles> _blobToolMock;
    public required Moq.Mock<ITalkToApi> _busToolMock;
    ...
    public void Setup()
    {
        _loggerMock = new Moq.Mock<ILogger<App.WatchInbound>>();
        _loggerMock.Setup(m =>
            m.Log(
                It.IsAny<LogLevel>(),
                It.IsAny<EventId>(),
        ...
        _busToolMock = new Moq.Mock<ITalkToApi>();
    ...
    public async Task RunAsync_CancellationAfterAdd_ThrowsBeforeMove()
    {
        ...
        _busToolMock.Verify(
            x => x.SendMessageAsync(It.IsAny<ApiRequest>(), It.IsAny<CancellationToken>()),
            Times.Never
        );
    }
    ...
    public async Task RunAsync_GeneratesUniqueMessageIdPerCall()
    {
        ...
        Assert.AreNotEqual(
            capturedIds[0],
            capturedIds[1],
            "Each call must produce a unique MessageId"
        );
    ...
    public async Task RunAsync_PayloadFileNameMatchesBlobName()
    {
        ...
        _busToolMock
            .Setup(x => x.SendMessageAsync(It.IsAny<ApiRequest>(), It.IsAny<CancellationToken>()))
            .Callback<ApiRequest, CancellationToken>(
                (req, _) => capturedPayload = req.Body as ApiAddPayload
            )
            .Returns(Task.CompletedTask);
