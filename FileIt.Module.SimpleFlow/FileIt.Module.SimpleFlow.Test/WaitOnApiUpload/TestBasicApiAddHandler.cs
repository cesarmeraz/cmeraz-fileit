...
public class TestBasicApiAddHandler
{
    ...
    private static string BuildMessage(string? correlationId, int nodeId = 99)
    {
    ...
    public async Task RunAsync_HappyPath_MovesBlobAndStampsApiId()
    {
        ...
        await target.RunAsync(correlationId, BuildMessage(correlationId, nodeId: 555));
    ...
    public async Task RunAsync_RequestLogMissing_Throws()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() =>
            target.RunAsync(Guid.NewGuid().ToString(), BuildMessage(Guid.NewGuid().ToString()))
        );
    }
    ...
    public async Task RunAsync_BlobNameMissing_Throws()
    {
        ...
        await Assert.ThrowsAsync<Exception>(() =>
            target.RunAsync(correlationId, BuildMessage(correlationId))
        );
    }
    ...
    public async Task RunAsync_RequestLogMissing_DoesNotMoveBlob()
    {
        ...
        try
        {
            await target.RunAsync(
                ...
                BuildMessage(Guid.NewGuid().ToString())
            );
        }
        catch (Exception) { }
    ...
    public async Task RunAsync_CancellationRequested_ThrowsBeforeMovingBlob()
    {
        ...
        await Assert.ThrowsAsync<OperationCanceledException>(() =>
            target.RunAsync(correlationId, BuildMessage(correlationId), cts.Token)
        );
    ...
    public async Task RunAsync_NullCorrelationId_LooksUpWithEmptyString()
    {
        ...
        await target.RunAsync(string.Empty, BuildMessage(correlationId: null));
