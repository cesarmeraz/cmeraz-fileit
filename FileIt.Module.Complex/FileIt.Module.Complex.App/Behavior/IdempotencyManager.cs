...
public sealed record IdempotencyOutcome(
    ...
    IdempotencyCachedResponse? Hit,
    string? RejectReason
);

/// <summary>
/// App-facing replay payload so Host can stay decoupled from Domain contracts.
/// </summary>
...
public class IdempotencyManager : IIdempotencyManager
{
    ...
    )
    {
        ...
        {
            _logger.LogWarning(
                ComplexEvents.IdempotencyKeyRejected,
                "Idempotency-Key too long: {Length} > {Max}",
                idempotencyKey.Length,
                _config.Idempotency.MaxKeyLength
            );
        ...
        {
            _logger.LogWarning(
                ComplexEvents.IdempotencyKeyConflict,
                "Idempotency-Key {Key} reused with a different request body",
                idempotencyKey
            );
            return new IdempotencyOutcome(
                IdempotencyState.Conflict,
                ToCachedResponse(hit),
                "Idempotency-Key has already been used with a different request payload."
            );
        }

        _logger.LogInformation(
            ComplexEvents.IdempotentReplay,
            "Replaying cached response for Idempotency-Key {Key} (status {Status})",
            idempotencyKey,
            hit.ResponseStatusCode
        );
        return new IdempotencyOutcome(IdempotencyState.Replay, ToCachedResponse(hit), null);
    }

    private static IdempotencyCachedResponse ToCachedResponse(IdempotencyHit hit)
    {
        return new IdempotencyCachedResponse(
            hit.ResponseStatusCode,
            hit.ResponseBody,
            hit.ResponseLocation
        );
    }
