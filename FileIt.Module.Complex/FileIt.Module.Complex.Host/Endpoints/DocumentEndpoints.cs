...
public class DocumentEndpoints
{
    ...
    )
    {
            ...
            case IdempotencyState.Invalid:
                return Problem(
                    ProblemDetailsFactory.BadRequest(
                        idemp.RejectReason ?? "Invalid Idempotency-Key.",
                        instance: req.Path
                    )
                );
        ...
        {
            return Problem(
                ProblemDetailsFactory.BadRequest("Request body is required.", instance: req.Path)
            );
        }
        ...
        {
            await _idempotency
                .SaveAsync(idempKey, rawBody, 201, responseJson, location, ct)
    ...
    )
    {
        ...
        string? nameFilter = req.Query.TryGetValue("name", out var nameVals)
            ? nameVals.ToString()
            : null;
    ...
    private static IActionResult ReplayCached(IdempotencyCachedResponse hit)
    {
        return new ContentResult
        {
